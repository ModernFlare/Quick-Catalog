...
import { MessageSquare, CheckCircle, Loader2, UserCheck } from "lucide-react";
...
const Feedback = () => {
  ...
  useEffect(() => {
    ...
    // Email берём напрямую из аккаунта
    if (user.email) setEmail(user.email);

    // Имя ищем сначала в профиле, потом в метаданных
    const fillName = async () => {
      ...
      const resolved = profileName || metaName || "";
    ...
    return (
      <main className="min-h-screen flex items-center justify-center px-4 pt-20 pb-8 bg-background">
        <div className="text-center animate-fade-slide-up">
          ...
          <p className="text-muted-foreground mb-6">Ваше сообщение получено. Мы свяжемся с вами в ближайшее время.</p>
          <Button onClick={handleReset}>Отправить ещё</Button>
        </div>
      </main>
    );
  }
    ...
    <main className="min-h-screen flex items-center justify-center px-4 pt-20 pb-8 bg-background">
      <Card className="w-full max-w-lg animate-fade-slide-up">
        <CardHeader className="text-center space-y-2">
          ...
          {autoFilled && (
            <div className="flex items-center justify-center gap-1.5 text-xs text-primary font-medium pt-1">
